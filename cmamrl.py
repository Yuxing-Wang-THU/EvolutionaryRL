...
class CMAESAgent:
    def __init__(self, env, env_seed, hl1, hl2, sigma, lamb, mu, mem_buff):
        self.env = env
...
class CMAMRLAgent:
    def __init__(self, env_name, env_seed = np.random.randint(1000), hl1 = 48, hl2 = 48, sigma = 0.5, lamb = 50, mu = 10, gamma = 0.95, lr = 0.00005, memory_size = 100000, batch_size = 32):
        env = gym.make(env_name)
        self.dqn_agent = DQNAgent(env.observation_space.shape[0], hl1, hl2, env.action_space.n, gamma, lr)
        self.es_agent = CMAESAgent(env, env_seed, hl1, hl2, sigma, lamb, mu, MemoryBuffer(memory_size, batch_size))
    ...
    def train(self, num_run):
        ...
        np.savetxt("CMAMRL-{}-seed-{}-rewards".format(self.es_agent.env.unwrapped.spec.id, self.es_agent.env_seed), rewards)       

    # Utility methods
    def get_weights(self): return self.es_agent.get_weights()
    def set_weights(self, theta): self.es_agent.set_weights(theta)
    def save_model(self): np.savetxt("CMAMRL-{}-seed-{}-weights".format(self.es_agent.env.unwrapped.spec.id, self.es_agent.env_seed), self.get_weights())
    def load_model(self, file_name): self.set_weights(np.loadtxt(file_name))
